...
const Game = () => {
  ...
  const startTimer = () => {
    ...
    timerRef.current = setInterval(() => {
      ...
      // Actualizar tiempo según el turno actual
      if (game.turn() === "w") {
        setWhiteTime((prev) => {
    ...
    }, 1000); // Intervalo de 100ms para mayor suavidad
  };
  ...
  const handleTimeOut = (color: "w" | "b") => {
    stopTimer();
    console.log(
      `¡Tiempo agotado para las ${color === "w" ? "blancas" : "negras"}!`
    );
  };
